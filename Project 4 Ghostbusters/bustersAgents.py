...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        def getMazeDistanceToGhost(ghostPosition):
            return self.distancer.getDistance(pacmanPosition, ghostPosition)

        def getMazeDistanceForAction(action):
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            return self.distancer.getDistance(successorPosition, closestGhostPos)

        livingGhostsLikelyPos = []
        for distribution in livingGhostPositionDistributions:
            mostLikelyPosition = distribution.argMax() 
            livingGhostsLikelyPos.append(mostLikelyPosition)


        if livingGhostsLikelyPos:
            closestGhostPos = min(livingGhostsLikelyPos, key=getMazeDistanceToGhost)
        else:
            return 'Stop' if 'Stop' in legal else legal[0]

        bestAction = min(legal, key=getMazeDistanceForAction)
