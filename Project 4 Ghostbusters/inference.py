...
import hunters
from util import manhattanDistance, raiseNotDefined, Counter as UtilCounter
...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = range(MAX_NOISE + X_RANGE + Y_RANGE - 1)

    variableDomainsDict[OBS1] = range(MAX_NOISE + X_RANGE + Y_RANGE - 1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Iterate through each variable in the elimination order
        #for joinVariable in bayesNet.variablesSet():
        for eliminationVariable in eliminationOrder:
            ...
            # Eliminate the variable if more than one unconditioned variable
            if len(joinedFactor.unconditionedVariables()) != 1:
                unconditionedFactor = eliminate(joinedFactor, eliminationVariable)
                currentFactorsList.append(unconditionedFactor)

        newFactor = joinFactors(currentFactorsList)

        # probabilities sum to 1
        inferenceResult = normalize(newFactor)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        #One consequence of this is that if the ghost’s position is the jail position, then the observation is None with probability 1,
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            allPos[ghostPosition] = p * self.beliefs[ghostPosition]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # self.beliefs.normalize()
        newBeliefs = UtilCounter()  # A new distribution for updated beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # special case where all particles have zero weight
        if w.total() == 0:
            self.initializeUniformly(gameState)
        else:
            w.normalize()
            ...
            for _ in range(numParticles): 
                self.particles.append(w.sample())
